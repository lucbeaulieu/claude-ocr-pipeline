...
let watchPath = (("~/Documents/Claude-OCR-Tasks" as NSString).expandingTildeInPath)
let convertScript = (("~/Library/Scripts/tasks2reminders_convert.sh" as NSString).expandingTildeInPath)
...
func processFile(_ fullPath: String) {
    ...
    guard fileName.hasSuffix(".json"), !fileName.hasPrefix(".") else { return }
    lock.lock()
    guard !inProgress.contains(fileName) else {
        lock.unlock()
        print("tasks2reminders_watcher: skip duplicate → \(fileName)")
        fflush(stdout)
        return
    }
    ...
    lock.unlock()
    print("tasks2reminders_watcher: processing → \(fileName)")
    fflush(stdout)
    let task = Process()
    ...
    try? task.run()
    task.waitUntilExit()
    print("tasks2reminders_watcher: done (exit \(task.terminationStatus))")
    fflush(stdout)
    lock.lock(); inProgress.remove(fileName); lock.unlock()
}

let queue = DispatchQueue(label: "com.pipeline.tasks2reminders.fsevents")

if let existing = try? fm.contentsOfDirectory(atPath: watchPath) {
    let jsonFiles = existing.filter { $0.hasSuffix(".json") }
    lock.lock(); knownFiles = Set(jsonFiles); lock.unlock()
    for f in jsonFiles { processFile((watchPath as NSString).appendingPathComponent(f)) }
}

let callback: FSEventStreamCallback = { _, _, _, _, _, _ in
    let current = Set((try? fm.contentsOfDirectory(atPath: watchPath))?.filter { $0.hasSuffix(".json") } ?? [])
...
pollTimer.setEventHandler {
    ...
    let jsonFiles = Set(current.filter { $0.hasSuffix(".json") })
    lock.lock(); let missed = jsonFiles.subtracting(knownFiles).subtracting(inProgress); lock.unlock()
    if !missed.isEmpty {
        print("tasks2reminders_watcher: polling caught \(missed.count) missed file(s)")
        fflush(stdout)
...
print("tasks2reminders_watcher: started — FSEvents + polling on \(watchPath)")
fflush(stdout)
dispatchMain()
